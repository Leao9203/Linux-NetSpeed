...
sh_ver="1.3.2"
cache="raw.githubusercontent.com/Leao9203/Linux-NetSpeed/master"
...
installbbr(){
	...
	if [[ "${release}" == "centos" ]]; then
		rpm --import http://${cache}/bbr/${release}/RPM-GPG-KEY-elrepo.org
		yum install -y http://${cache}/bbr/${release}/${version}/${bit}/kernel-ml-${kernel_version}.rpm
		yum remove -y kernel-headers
		yum install -y http://${cache}/bbr/${release}/${version}/${bit}/kernel-ml-headers-${kernel_version}.rpm
		yum install -y http://${cache}/bbr/${release}/${version}/${bit}/kernel-ml-devel-${kernel_version}.rpm
	elif [[ "${release}" == "debian" || "${release}" == "ubuntu" ]]; then
		mkdir bbr && cd bbr
		wget http://${cache}/libssl1.0.0_1.0.1t-1+deb8u12_amd64.deb
		wget -N --no-check-certificate http://${cache}/bbr/debian-ubuntu/linux-headers-${kernel_version}-all.deb
		wget -N --no-check-certificate http://${cache}/bbr/debian-ubuntu/${bit}/linux-headers-${kernel_version}.deb
		wget -N --no-check-certificate http://${cache}/bbr/debian-ubuntu/${bit}/linux-image-${kernel_version}.deb
...
installbbrplus(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		wget -N --no-check-certificate http://${cache}/bbrplus/debian-ubuntu/${bit}/linux-headers-${kernel_version}.deb
		wget -N --no-check-certificate http://${cache}/bbrplus/debian-ubuntu/${bit}/linux-image-${kernel_version}.deb
		dpkg -i linux-headers-${kernel_version}.deb
		dpkg -i linux-image-${kernel_version}.deb
		cd .. && rm -rf bbrplus
	fi
...
installlot(){
	if [[ "${release}" == "centos" ]]; then
		rpm --import http://${cache}/lotserver/${release}/RPM-GPG-KEY-elrepo.org
		yum remove -y kernel-firmware
		yum install -y http://${cache}/lotserver/${release}/${version}/${bit}/kernel-firmware-${kernel_version}.rpm
		yum install -y http://${cache}/lotserver/${release}/${version}/${bit}/kernel-${kernel_version}.rpm
		yum remove -y kernel-headers
		yum install -y http://${cache}/lotserver/${release}/${version}/${bit}/kernel-headers-${kernel_version}.rpm
		yum install -y http://${cache}/lotserver/${release}/${version}/${bit}/kernel-devel-${kernel_version}.rpm
	elif [[ "${release}" == "ubuntu" ]]; then
		bash <(wget --no-check-certificate -qO- "http://${cache}/Debian_Kernel.sh")
...
startbbrmod(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		wget -N --no-check-certificate http://${cache}/bbr/tcp_tsunami.c
...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "http://${cache}/tcp.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			wget -N --no-check-certificate http://${cache}/tcp.sh && chmod +x tcp.sh
